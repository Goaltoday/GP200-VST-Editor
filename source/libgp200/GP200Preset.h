...
{
  public:
    static GP200Preset decodeLivePresetDump (const juce::MemoryBlock& presetData);
  ...
  private:
    static juce::String readAsciiString (const juce::uint8* data,
                                         std::size_t dataSize,
                                         std::size_t offset,
                                         std::size_t maxLength);

    static juce::uint32 readUInt32LE (const juce::uint8* data, std::size_t dataSize, std::size_t offset);

    static float readFloat32LE (const juce::uint8* data, std::size_t dataSize, std::size_t offset);
};
} // namespace gp200
